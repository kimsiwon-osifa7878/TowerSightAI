...
from towersightai.inference.image_smoke import build_image_hailo_pipeline, image_smoke_command, run_image_hailo_smoke
...
def test_image_hailo_pipeline_uses_sample_image_hailo_callback_and_png_output(tmp_path: Path):
    ...
    assert "hailopython module=towersightai.inference.callback" in pipeline
    assert "hailooverlay" in pipeline
    assert f"pngenc ! filesink location={output}" in pipeline
