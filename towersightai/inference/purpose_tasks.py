...
PURPOSE_TASK_SPECS = {
    ...
    PURPOSE_LPR_IMAGE: PurposeTaskSpec(
        PURPOSE_LPR_IMAGE,
        "번호판 이미지 LPR",
        "tmp/car_number-test 이미지를 Hailo LPR 예제 파이프라인으로 실행합니다.",
    ),
...
def lpr_image_process(
    ...
    *,
    ...
    location = event_dir / "lpr-input-%05d.png"
    stop_index = len(prepared) - 1
    pipeline = " ".join(
        (
            ...
            "scheduler-threshold=5 scheduler-timeout-ms=100 batch-size=1 nms-score-threshold=0.3 "
            "nms-iou-threshold=0.45 output-format-type=HAILO_FORMAT_TYPE_FLOAT32",
            ...
            "scheduler-threshold=1 scheduler-timeout-ms=100 batch-size=1 output-format-type=HAILO_FORMAT_TYPE_FLOAT32",
    ...
    env = _gst_runtime_env(settings)
    env["TOWERSIGHTAI_LPR_EVENT_PATH"] = str(event_path)
    return PurposeInferenceProcess(
        ...
        event_path=event_path,
        model_paths=(
            resources / "yolov5m_vehicles.hef",
            resources / "tiny_yolov4_license_plates.hef",
            resources / "lprnet.hef",
        ),
...
def _prepare_lpr_images(image_dir: Path, event_dir: Path) -> tuple[Path, ...]:
    ...
    for index, source_path in enumerate(source_paths):
        target = event_dir / f"lpr-input-{index:05d}.png"
