...
class GStreamerApp:
    ...
    def get_pipeline_string(self):
        if (self.source_type == "rtsp"):
            source_element = f"rtspsrc location=rtsp://erumtni:erumtni@192.168.0.94:554/stream1 latency=50 name=src_0 ! "
            source_element += QUEUE("queue_rtsp", max_size_buffers=5)
            source_element += f"rtph264depay ! h264parse ! avdec_h264 max-threads=2 ! "
            source_element += f"video/x-raw,format=I420 ! "
        
        elif (self.source_type == "mipi"):
