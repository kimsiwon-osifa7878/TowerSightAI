...
def label_at(image, text: str, point: tuple[float, float], *, size=20, color=(255, 255, 255), anchor="lt"):
    height, width = image.shape[:2]
    return put_text(image, text, (int(point[0] * width), int(point[1] * height)), size=size, color=color, anchor=anchor)


def failure_note(image, reasons: Sequence[str], *, title: str = "판정 불가"):
    """실패 사유를 이미지 왼쪽 위에 한국어로 새긴다."""
    height, width = image.shape[:2]
    body = "\n".join(f"· {reason}" for reason in reasons) or "· 사유 미기록"
    text = f"[{title}]\n{body}"
    size = max(width // 85, 14)
    put_text(image, text, (int(width * 0.015), int(height * 0.04)), size=size, color=(150, 190, 255))
    import cv2
...
def side_panel(image, lines: Sequence[str], *, title: str = "측정값", size: int | None = None):
    """성공한 경우의 수치 요약을 오른쪽 위에 새긴다."""
    height, width = image.shape[:2]
    text = f"[{title}]\n" + "\n".join(lines)
    size = size or max(width // 65, 15)
    return put_text(image, text, (int(width * 0.985), int(height * 0.04)), size=size, anchor="rt")
...
def arrow_with_length(image, a: tuple[float, float], b: tuple[float, float], text: str, color, *, size=20):
    ...
    mid = ((ax + bx) // 2, (ay + by) // 2)
    put_text(image, text, mid, size=size, color=(255, 255, 255), anchor="ct")
    return image
