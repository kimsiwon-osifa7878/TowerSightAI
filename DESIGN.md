...
- Provides a visible `운영자 모드` service button in the bottom-right corner, above the bottom strip. Holding the hidden hotspot is impractical at the installed machine, so this button is the primary on-site operator entry point. It is a mode-switch control only: it carries no state, no diagnostics, and cannot change safety status or PLC output. It stays subdued (translucent slate surface, no red, no cyan guidance color) so it never competes with the driver instruction, and it is absent from the operator-mode driver preview.
