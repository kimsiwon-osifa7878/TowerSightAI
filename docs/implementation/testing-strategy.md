...
- Hailo auto-recovery policy (`test_hailo_recovery.py`): it waits for two minutes of *continuous* error, a single healthy sample clears the pending error, `degraded` alone never triggers it, each attempt restarts the clock so attempts are never back-to-back, the budget caps attempts per window while still giving a permanently dead device roughly three chances an hour, and it can be switched off entirely. UI (`test_pyqt_app.py`): the trigger fires only after the delay, is skipped while a recovery is already running or when the window has no settings, and final OK stays blocked.
- Hailo device recovery (`test_hailo_recovery.py`): the shell helper reports its plan under `--dry-run` without touching the driver or PCI device, fails clearly with no Hailo device present, refuses the real path without root, and the Python wrapper parses RESULT/DETAIL/STEP, explains a missing sudoers rule, and reports timeouts and OS errors instead of raising. UI (`test_pyqt_app.py`): a declined confirmation changes nothing, accepting stops inference before the driver unload, the button disables while running, and final OK stays blocked for success and failure.
- Repository hygiene (`test_repo_hygiene.py`): no `data/calibration/**.json` may be tracked by git, and the ignore rules must keep covering the runtime calibration paths (results, per-host copies, intrinsics, capture sessions). Calibration is shared over the NAS; a tracked result blocks `git pull` on a machine that has its own measurement.
