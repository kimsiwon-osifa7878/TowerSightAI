...
| 카메라 캘리브레이션 (공유) | `NAS로 공유` 이 장비에서 측정한 렌즈·지면 값을 NAS(`<폴더>/calibration/<장비이름>/`)에 올립니다. `NAS에서 가져오기` 다른 장비가 올린 값을 내려받습니다 — **체커보드를 들 수 없는 현장 장비에서 씁니다.** 가져온 값은 원래 측정 장비 이름을 유지해 점검표에 `△ 이 장비가 아니라 …에서 측정한 값`으로 표시되고, `reviewed=false`·`safe_to_operate=false`도 그대로입니다 |
| 지면 기준점 | **팔레트 네 모서리 + 주황 스토퍼 밑동**을 화면에서 클릭하면 카메라가 팔레트 기준으로 어디에(x·y·z) 어느 각도로(팬·틸트·롤) 있는지 계산합니다. 모서리 순서는 상관없고, 스토퍼가 "안쪽"을 정합니다. 투영된 팔레트와 500 mm 격자가 실제와 맞는지 보고 저장 → `data/calibration/ground/<camera>.json` (reviewed=false, 측정 파일일 뿐). 먼저 `카메라 캘리브레이션`에서 렌즈 측정이 끝나 있어야 합니다 |
| 카메라 캘리브레이션 | 체커보드(`towersightai-checkerboard`로 인쇄) 촬영 13자세. **화면에 뜨는 A4 가로 비율 사각 상자 안에 보드를 맞추고 3초 유지하면 자동 촬영** (상자를 벗어나면 이유를 한국어로 알려 주고 다시 3초). `측정 실행`으로 렌즈 내부 파라미터 측정 → `data/calibration/intrinsics/<camera>.json`(reviewed=false, 측정 파일일 뿐). `결과 확인`은 저장된 측정값의 품질 점검표와 **왜곡 보정 전/후 비교 이미지**(양쪽에 곧은 노란 격자를 겹침 — 보정 후 실제 직선이 격자와 나란하면 잘 된 것)를 보여 줍니다 |
...
- 번호판은 **뒷자리 4자리만** 신뢰한다. OCR 모델에 한글이 없어 가운데 글자가 라틴 문자로 바뀌고 앞뒤 숫자까지
  밀리기 때문이다. 번호판 상자의 오른쪽을 여러 지점에서 잘라 다시 읽고 다수결로 4자리를 정한다. 4자리가 안 나오면
  추측하지 않고 그 판독을 버린다. 전체 문자열은 `plate_text`로 확인용만 남는다.
- 차량이 감지되면 먼저 **입고인지 출고인지 판별**한다. 번호판이 읽히거나 전면 카메라의 차량 박스가 좁고 서 있으면 입고,
  박스가 화면을 가득 채우며 옆으로 누우면 출고다. 출고 중에는 사용자 화면에 `출고중`만 뜨고 사람이 감지돼도 경고하지 않는다.
  번호판도 없고 형태도 불확실하면 진입으로 확정하지 않고 오류로 처리한다. 판정 임계값은 `감시 설정` 페이지에서 조정한다.
- 스냅샷·클립 파일명은 한국시간 날짜와 시각에 `_kr`을 붙인다: `20260916-154634-702684_kr-plate-crop-front.jpg`.
  2026-09-16 이전 파일은 이름이 UTC라 한국시간보다 9시간 빠르게 보인다(폴더 날짜는 예전부터 한국시간 기준).
